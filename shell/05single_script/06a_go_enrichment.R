...
write_go_result <- function(layer_id, comparison_id, cluster_id, direction, ont, source_species, deg_source, deg_status, genes, org_db, enrichment_source) {
  ...
  if (!clusterprofiler_available_06()) {
    ...
    barplot <- ""
    if (nrow(result_df) > 0) {
      plot_title <- sprintf("%s %s %s %s %s %s", layer_id, comparison_id, cluster_id, direction, ont, source_species)
  ...
  barplot <- ""
  if (nrow(result_df) > 0) {
    plot_title <- sprintf("%s %s %s %s %s", layer_id, comparison_id, cluster_id, direction, ont)
