...
read_cfg <- function() {
  list(
    ...
    ortholog_cache_dir = Sys.getenv("ORTHOLOG_CACHE_DIR", file.path(must_getenv("RESULTS_DIR"), "ortholog_cache")),
    cell_cycle_genes_rds = Sys.getenv("CELL_CYCLE_GENES_RDS", file.path(Sys.getenv("ORTHOLOG_CACHE_DIR", file.path(must_getenv("RESULTS_DIR"), "ortholog_cache")), "chicken_cc_genes.rds")),
...
resolve_cell_cycle_gene_sets <- function(cfg) {
  candidates <- unique(c(
    normalize_scalar_value(cfg$cell_cycle_genes_rds),
    file.path(normalize_scalar_value(cfg$ortholog_cache_dir), "chicken_cc_genes.rds")
  ))
  candidates <- candidates[nzchar(candidates)]
  ...
  for (path in candidates) {
    ...
    if (is.list(payload) && all(c("s.genes", "g2m.genes") %in% names(payload))) {
      return(list(
        s.genes = unique(as.character(payload$s.genes)),
        g2m.genes = unique(as.character(payload$g2m.genes)),
        source = path
      ))
    }
  }

  fallback <- tryCatch(Seurat::cc.genes.updated.2019, error = function(e) NULL)
  if (is.null(fallback)) {
    return(NULL)
  }
  list(
    s.genes = unique(as.character(fallback$s.genes)),
    g2m.genes = unique(as.character(fallback$g2m.genes)),
    source = "Seurat::cc.genes.updated.2019"
  )
}
...
resolve_feature_context <- function(gene_names, cfg, declared_gene_id_type = "auto") {
  ...
  cc_genes <- resolve_cell_cycle_gene_sets(cfg)
