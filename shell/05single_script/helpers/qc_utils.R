...
load_cell_cycle_genes_local <- function(cfg) {
  ...
  mapped <- tryCatch(readRDS(cc_path), error = function(e) {
    stop(sprintf("无法读取 cc_genes.rds: %s。请先运行 shell/03stages/00_ortholog.sh", cc_path), call. = FALSE)
  })
...
require_ortholog_cc_genes_local <- function(cfg) {
  if (!file.exists(cfg$ortholog_manifest_path)) {
    stop(sprintf("缺少 cc_genes.rds，请先运行 shell/03stages/00_ortholog.sh。缺少 manifest: %s", cfg$ortholog_manifest_path), call. = FALSE)
  }
  ...
  cc_path <- tryCatch(resolve_output_local(manifest, "cc_genes"), error = function(e) {
    stop(sprintf("缺少 cc_genes.rds，请先运行 shell/03stages/00_ortholog.sh。ortholog manifest 未暴露 cc_genes: %s", cfg$ortholog_manifest_path), call. = FALSE)
  })
  if (!file.exists(cc_path)) {
    stop(sprintf("缺少 cc_genes.rds，请先运行 shell/03stages/00_ortholog.sh。manifest 指向的文件不存在: %s", cc_path), call. = FALSE)
  }
  cc_path
}
