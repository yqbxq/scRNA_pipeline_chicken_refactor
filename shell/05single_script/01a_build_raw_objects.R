...
resolve_matrix_dir <- function(cfg, sample_id, inventory_row) {
  ...
  if (!is.null(inventory_row) && "filtered_matrix_dir" %in% colnames(inventory_row)) {
    fallback <- normalize_scalar_value(inventory_row$filtered_matrix_dir[1])
...
sample_builds <- lapply(sample_ids, function(sample_id) {
  ...
  counts <- pick_expression_matrix(Seurat::Read10X(data.dir = matrix_dir))
