...
module_name <- "06b_kegg_enrichment"
prepare_dirs_06(cfg)
...
write_kegg_result <- function(layer_id, comparison_id, cluster_id, direction, source_species, organism, deg_source, deg_status, genes, org_db, enrichment_source) {
  ...
  run_res <- run_kegg_enrichment_single(
...
for (idx in seq_len(nrow(input_grid))) {
  ...
  for (cluster_id in cluster_ids) {
    ...
    for (direction in names(gene_sets)) {
      ...
      manifest_rows[[length(manifest_rows) + 1]] <- write_kegg_result(
        layer_id = layer_id,
        comparison_id = comparison_id,
        cluster_id = cluster_id,
        direction = direction,
...
write_tsv_local(manifest_df, cfg$kegg_enrichment_manifest_tsv)

report_path <- file.path(cfg$enrichment_report_dir, "06b_kegg_report.md")
...
report_lines <- c(
  "# 06b KEGG Enrichment",
  "",
  sprintf("- manifest: `%s`", cfg$kegg_enrichment_manifest_tsv),
  ...
  "",
  "## Low Mapping Rows",
  if (nrow(low_mapping) == 0) "No rows below 30% ID mapping." else render_markdown_table_local(head(low_mapping[, c("layer_id", "comparison_id", "cluster_id", "gene_direction", "source_species", "mapping_rate", "status"), drop = FALSE], 100))
)
write_markdown_local(report_lines, report_path)

if (file.exists(cfg$module_06b_manifest_path)) {
  unlink(cfg$module_06b_manifest_path)
}
write_manifest_local(
  manifest_path = cfg$module_06b_manifest_path,
  new_outputs = list(
    kegg_enrichment_manifest_tsv = build_output_entry(cfg$kegg_enrichment_manifest_tsv, "tsv", module_name, "one row per layer/comparison/cluster/direction/species KEGG result", base_dir = cfg$project_root, schema = infer_schema_from_df(manifest_df)),
    report = build_output_entry(report_path, "md", module_name, "KEGG enrichment report", base_dir = cfg$project_root)
  ),
...
message("06b completed. manifest: ", cfg$kegg_enrichment_manifest_tsv)
