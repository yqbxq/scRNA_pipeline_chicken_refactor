...
usage() {
  cat <<'EOF'
Usage:
  bash shell/01run.sh <stage>

Examples:
  bash shell/01run.sh 08_regulation
  bash shell/01run.sh shell/03stages/08_regulation.sh
EOF
}
...
case "${stage_arg}" in
  */*)
    ...
    ;;
  *.sh)
    stage_path="${SHELL_ROOT}/03stages/${stage_arg}"
    ;;
  *)
    stage_path="${SHELL_ROOT}/03stages/${stage_arg}.sh"
    ;;
esac
