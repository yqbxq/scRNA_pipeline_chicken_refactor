...
run_stage_if_stale \
  "${SHELL_ROOT}/05single_script/02a_ambient_branch.R" \
  "${MODULE_02A_MANIFEST}" \
  "${MODULE_01A_MANIFEST}" \
  "${RAW_OBJECT}" \
...
run_stage_if_stale \
  "${SHELL_ROOT}/05single_script/02b1_qc_filter.R" \
...
run_stage_if_stale \
  "${SHELL_ROOT}/05single_script/02b2_doublet.R" \
...
run_stage_if_stale \
  "${SHELL_ROOT}/05single_script/02c_post_qc_eda.R" \
  "${MODULE_02C_MANIFEST}" \
  "${MODULE_01A_MANIFEST}" \
  "${MODULE_01B_MANIFEST}" \
  "${MODULE_02A_MANIFEST}" \
