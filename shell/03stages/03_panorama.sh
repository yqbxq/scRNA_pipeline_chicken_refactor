...
run_stage_if_stale \
  "${SHELL_ROOT}/05single_script/03a1_normalize_hvg.R" \
...
run_stage_if_stale \
  "${SHELL_ROOT}/05single_script/03a2_reduce_integrate.R" \
  "${MODULE_03A2_MANIFEST}" \
  "${MODULE_03A1_MANIFEST}" \
  "${OBJECT_LAYER_CONFIG_FILE}"

run_stage_if_stale \
  "${SHELL_ROOT}/05single_script/03b_integration_eda.R" \
  "${MODULE_03B_MANIFEST}" \
  "${MODULE_03A2_MANIFEST}"
...
run_stage_if_stale \
  "${SHELL_ROOT}/05single_script/03c_cluster.R" \
  "${MODULE_03C_MANIFEST}" \
  "${MODULE_03A2_MANIFEST}" \
  "${MODULE_03B_MANIFEST}" \
...
run_stage_if_stale \
  "${SHELL_ROOT}/05single_script/03d_annotate.R" \
  "${MODULE_03D_MANIFEST}" \
  "${MODULE_03C_MANIFEST}" \
  "${MARKER_PANEL_DIR}"

run_stage_if_stale \
  "${SHELL_ROOT}/05single_script/03e_annotation_eda.R" \
  "${MODULE_03E_MANIFEST}" \
  "${MODULE_03D_MANIFEST}"
...
update_workflow_status \
  ...
  "review ${EDA_GATE_FILE} and approve annotation before bash ${PIPELINE_ROOT}/shell/03stages/04_subcluster.sh" \
