...
run_comm_stage_if_stale \
  "${SHELL_ROOT}/05single_script/07a_cellchat.R" \
  "${MODULE_07A_MANIFEST}" \
  "${MODULE_03D_MANIFEST}" \
  "${MODULE_04B_MANIFEST}" \
  "${MODULE_00_MANIFEST}" \
...
if [[ "${RUN_NICHENET:-yes}" != "no" ]]; then
  if is_stale_output "${MODULE_07B_MANIFEST}" \
    ...
    echo "运行 ${SHELL_ROOT}/05single_script/07b_nichenet.R"
    if run_r_interaction "${SHELL_ROOT}/05single_script/07b_nichenet.R"; then
      COMMUNICATION_RERAN=1
    else
...
run_comm_stage_if_stale \
  "${SHELL_ROOT}/05single_script/07c_communication_eda.R" \
  "${MODULE_07C_MANIFEST}" \
  "${MODULE_07A_MANIFEST}" \
  "${MODULE_07B_MANIFEST}" \
  "${COMMUNICATION_PAIRS_SHEET}"
