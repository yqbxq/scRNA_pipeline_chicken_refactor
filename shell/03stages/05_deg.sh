...
run_deg_stage_if_stale \
  "${SHELL_ROOT}/05single_script/05a_marker_discovery.R" \
  "${MODULE_05A_MANIFEST}" \
  "${LAYER_STATUS_FILE}" \
  "${COMPARISON_SHEET}" \
  "${MODULE_03D_MANIFEST}"
...
run_deg_stage_if_stale \
  "${SHELL_ROOT}/05single_script/05b_pseudobulk_de.R" \
  "${MODULE_05B_MANIFEST}" \
  "${MODULE_05A_MANIFEST}" \
  "${LAYER_STATUS_FILE}" \
  "${COMPARISON_SHEET}"
...
run_deg_stage_if_stale \
  "${SHELL_ROOT}/05single_script/05d_deg_eda.R" \
  "${MODULE_05D_MANIFEST}" \
  "${MODULE_05A_MANIFEST}" \
  "${MODULE_05B_MANIFEST}" \
  "${MODULE_05C_MANIFEST}"
...
update_workflow_status \
  "05_deg_completed" \
  "review ${DEG_REPORT} and approve deg before bash ${PIPELINE_ROOT}/shell/03stages/06_enrichment.sh" \
