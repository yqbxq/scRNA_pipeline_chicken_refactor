...
def resolve_dnbelab_root(
    sample_id: str,
    source_path: Path,
    data_dir: Path,
    dnbc4tools_out_dir: Path | None = None,
) -> Path | None:
    candidates = [
        source_path,
    ...
    if dnbc4tools_out_dir is not None:
        candidates.extend((dnbc4tools_out_dir / sample_id, dnbc4tools_out_dir))
    for candidate in unique_paths(candidates):
...
def detect_platform(
    ...
    dnbelab_root = resolve_dnbelab_root(sample_id, source_path, data_dir, dnbc4tools_out_dir)
