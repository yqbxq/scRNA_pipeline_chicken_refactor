...
[[ -x "${R_BIN}" ]] || {
  echo "Missing Rscript for smoke_deg_shell_05: ${R_BIN}" >&2
  exit 1
}
...
"${R_BIN}" "${PIPELINE_ROOT}/shell/05single_script/05a_marker_discovery.R"
"${R_BIN}" "${PIPELINE_ROOT}/shell/05single_script/05b_pseudobulk_de.R"
"${R_BIN}" "${PIPELINE_ROOT}/shell/05single_script/05c_composition.R"
"${R_BIN}" "${PIPELINE_ROOT}/shell/05single_script/05d_deg_eda.R"
