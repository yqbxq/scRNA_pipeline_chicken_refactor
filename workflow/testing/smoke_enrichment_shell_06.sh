...
[[ -x "${R_BIN}" ]] || {
  echo "Missing Rscript for smoke_enrichment_shell_06: ${R_BIN}" >&2
  exit 1
}
...
EOF
then
  "${R_BIN}" "${PIPELINE_ROOT}/shell/05single_script/06a_go_enrichment.R"
  "${R_BIN}" "${PIPELINE_ROOT}/shell/05single_script/06b_kegg_enrichment.R"
...
EOF
  ...
  ENRICHMENT_MIN_INPUT_GENES=9999 "${R_BIN}" "${PIPELINE_ROOT}/shell/05single_script/06a_go_enrichment.R"
  ENRICHMENT_MIN_INPUT_GENES=9999 "${R_BIN}" "${PIPELINE_ROOT}/shell/05single_script/06b_kegg_enrichment.R"
...
EOF
    ...
    echo "smoke_enrichment_shell_06: skipping direct clusterProfiler runner check; clusterProfiler is not available" >&2
  fi
else
  echo "smoke_enrichment_shell_06: skipping 06a/06b script checks; enrichment R packages are not available" >&2
fi
...
"${R_BIN}" "${PIPELINE_ROOT}/shell/05single_script/06c_enrichment_eda.R"
...
cat("smoke_enrichment_shell_06: PASS\n")
EOF
