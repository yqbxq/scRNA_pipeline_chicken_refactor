...
if main_upstream_ready == "true" and standardized_ok:
    next_step = f"bash {pipeline_root}/workflow/01_install_envs.sh"
elif main_upstream_ready != "true":
    if project_input_modes == {"fastq"}:
        next_step = f"bash {pipeline_root}/workflow/10_run_cellranger_from_fastq.sh"
    else:
        next_step = f"bash {pipeline_root}/workflow/03_audit_inputs.sh"
else:
    next_step = f"bash {pipeline_root}/workflow/04_standardize_inputs.sh"
