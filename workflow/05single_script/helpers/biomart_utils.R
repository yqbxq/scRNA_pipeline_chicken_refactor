build_homolog_attributes <- function(mart, species_key) {
  prefix <- species_prefix(species_key)
  available_attrs <- biomaRt::listAttributes(mart)$name
  desired_attrs <- c(
    "external_gene_name",
    "ensembl_gene_id",
  ...
  desired_attrs[desired_attrs %in% available_attrs]
}
...
query_one_chunk <- function(mart, gene_chunk, species_key, filter_name = "external_gene_name") {
  attrs <- build_homolog_attributes(mart, species_key)
  biomaRt::getBM(
    attributes = attrs,
    filters = filter_name,
    values = gene_chunk,
...
query_one_chunk_with_retry <- function(gene_chunk, chunk_idx, total_chunks, species_key, mirrors, filter_name = "external_gene_name", max_attempts = 6L) {
  last_err <- NULL
  for (attempt in seq_len(max_attempts)) {
    ...
    res <- tryCatch({
      ...
      query_one_chunk(mart, gene_chunk, species_key, filter_name = filter_name)
    }, error = function(e) {
      last_err <<- e
      NULL
    })
