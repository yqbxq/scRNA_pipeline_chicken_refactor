...
module_name <- "spatial_08b_pyscenic"
prepare_dirs_spatial(cfg)
out_dir <- file.path(cfg$spatial_table_dir, "spatial_regulation", "pyscenic")
manifest_path <- file.path(cfg$manifest_dir, module_name, "_manifest.json")
ensure_dir(out_dir)
cmd <- c(
  file.path(cfg$pipeline_root, "workflow", "04python", "spatial_pyscenic.py"),
...
st06_write_manifest_local(
  manifest_path,
  list(spatial_pyscenic_h5ad_manifest = build_output_entry(manifest_tsv, "tsv", module_name, "H5AD-first spatial pySCENIC manifest", base_dir = cfg$project_root, schema = infer_schema_from_df(df))),
  module_name,
  cfg$project_root,
  inputs = list(spatial_h5ad_module = Sys.getenv("SPATIAL_REGULATION_H5AD_MODULE", unset = "spatial_03_region")),
