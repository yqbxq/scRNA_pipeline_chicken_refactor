...
module_name <- "spatial_07b_deconvolution_transfer"
manifest_df <- run_deconv_method_scaffold_st(cfg, "transfer", "Seurat", cfg$spatial_transfer_table_dir, "failed_transfer_data")
manifest_tsv <- file.path(cfg$spatial_transfer_table_dir, "transfer_manifest.tsv")
report_path <- file.path(cfg$spatial_transfer_figure_dir, "transfer_report.md")
...
write_deconv_module_manifest_st(
  ...
  report_path,
  module_name,
  cfg$module_07b_deconvolution_transfer_manifest_path,
  "transfer_manifest",
  depends_on = list(spatial_07a_deconvolution_rctd = cfg$module_07a_deconvolution_rctd_manifest_path)
)

message(sprintf("spatial TransferData deconvolution complete: %d manifest rows", nrow(manifest_df)))
