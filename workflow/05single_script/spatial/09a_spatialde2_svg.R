...
module_name <- "spatial_09a_spatialde2_svg"
prepare_dirs_spatial(cfg)
out_dir <- file.path(cfg$spatial_table_dir, "09_svg", "spatialde2")
manifest_path <- file.path(cfg$manifest_dir, module_name, "_manifest.json")
ensure_dir(out_dir)
cmd <- c(
  file.path(cfg$pipeline_root, "workflow", "04python", "spatialde2_svg.py"),
  "--st-module", Sys.getenv("SPATIAL_SVG_H5AD_MODULE", unset = "spatial_03_region"),
  "--section-id", Sys.getenv("SPATIAL_SVG_SECTION_ID", unset = ""),
  "--out-dir", out_dir,
...
manifest_tsv <- file.path(out_dir, "spatialde2_manifest.tsv")
if (!file.exists(manifest_tsv)) {
  st06_write_tsv(data.frame(status = "failed_sidecar", reason = paste(as.character(run), collapse = " "), input_h5ad_path = "", svg_tsv = "", stringsAsFactors = FALSE), manifest_tsv)
}
df <- st06_read_tsv(manifest_tsv)
st06_write_manifest_local(
  manifest_path,
  list(spatialde2_manifest = build_output_entry(manifest_tsv, "tsv", module_name, "H5AD-first SpatialDE2 manifest", base_dir = cfg$project_root, schema = infer_schema_from_df(df))),
  module_name,
  cfg$project_root,
  inputs = list(spatial_h5ad_module = Sys.getenv("SPATIAL_SVG_H5AD_MODULE", unset = "spatial_03_region")),
