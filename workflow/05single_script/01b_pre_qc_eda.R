...
for (i in seq_len(nrow(sample_summary))) {
  ...
  if (row$frac_below_nfeature_cutoff > cfg$triage_frac_below_cutoff || row$frac_below_ncount_cutoff > cfg$triage_frac_below_cutoff) {
    append_triage(sample_id, "high", "low_complexity_burden", sprintf("frac_below_nfeature=%.3f; frac_below_ncount=%.3f", row$frac_below_nfeature_cutoff, row$frac_below_ncount_cutoff))
  }
  if (identical(row$mito_detection_method, "failed") || isTRUE(row$mito_feature_count == 0)) {
    append_triage(sample_id, "high", "mito_detection_failed", sprintf("mito_detection_method=%s; mito_feature_count=%s; detail=%s", normalize_scalar_value(row$mito_detection_method, "failed"), normalize_scalar_value(row$mito_feature_count, "0"), normalize_scalar_value(row$mito_detection_detail, "NA")))
  } else {
    if (identical(row$mito_detection_method, "prefix_fallback")) {
      append_triage(sample_id, "medium", "mito_prefix_fallback", sprintf("detail=%s; genes=%s", normalize_scalar_value(row$mito_detection_detail, "NA"), normalize_scalar_value(row$mito_detected_gene_names, "NA")))
    }
    if (grepl("mito_feature_count_low", normalize_scalar_value(row$mito_warning_codes), fixed = TRUE)) {
      append_triage(sample_id, "medium", "mito_feature_count_low", sprintf("mito_feature_count=%s; genes=%s", normalize_scalar_value(row$mito_feature_count, "NA"), normalize_scalar_value(row$mito_detected_gene_names, "NA")))
    }
  }
  if (row$frac_above_mito_cutoff > cfg$triage_frac_above_mito) {
    append_triage(sample_id, "high", "high_mito_burden", sprintf("frac_above_mito_cutoff=%.3f; median_percent_mito=%.3f", row$frac_above_mito_cutoff, row$median_percent_mito))
  }
  if (row$nfeature_density_peaks > cfg$triage_density_peaks || row$mito_density_peaks > cfg$triage_density_peaks) {
    append_triage(sample_id, "medium", "possible_multimodal_qc", sprintf("nfeature_peaks=%s; mito_peaks=%s", row$nfeature_density_peaks, row$mito_density_peaks))
  }
  if (!is.na(row$median_genes_per_cell) && !is.na(row$sequencing_saturation) && row$median_genes_per_cell < 800) {
    if (row$sequencing_saturation < 60) {
      append_triage(sample_id, "medium", "low_genes_low_saturation", sprintf("median_genes_per_cell=%.1f; sequencing_saturation=%.1f", row$median_genes_per_cell, row$sequencing_saturation))
    } else if (row$sequencing_saturation >= 80) {
      append_triage(sample_id, "medium", "low_genes_high_saturation", sprintf("median_genes_per_cell=%.1f; sequencing_saturation=%.1f", row$median_genes_per_cell, row$sequencing_saturation))
    }
  }
  if (identical(row$ambient_soupx_ready, "true")) {
    append_triage(sample_id, "info", "ambient_soupx_ready", sprintf("raw_matrix_kind=%s; preferred_method=%s", normalize_scalar_value(row$raw_matrix_kind, "NA"), normalize_scalar_value(row$ambient_preferred_method, "soupx")))
  } else if (identical(row$ambient_decontx_ready, "true")) {
    append_triage(sample_id, "info", "ambient_decontx_fallback", sprintf("preferred_method=%s; ambient_notes=%s", normalize_scalar_value(row$ambient_preferred_method, "decontx"), normalize_scalar_value(row$ambient_notes, "NA")))
  }
  if (identical(row$gene_id_type_resolved, "unknown")) {
    append_triage(sample_id, "high", "gene_id_type_unrecognized", sprintf("feature_name_profile=%s; mito_feature_count=%s", row$feature_name_profile, row$mito_feature_count))
  }
  if (identical(row$ambient_soupx_ready, "false")) {
    append_triage(sample_id, "info", "raw_matrix_missing_for_soupx", sprintf("raw_matrix_dir=%s; preferred_method=%s", ifelse(nzchar(row$raw_matrix_dir), row$raw_matrix_dir, "NA"), normalize_scalar_value(row$ambient_preferred_method, "none")))
  }
  if (length(describe_readiness_notes(row$readiness_notes)) > 0) {
    append_triage(sample_id, "info", "input_contract_limits", paste(describe_readiness_notes(row$readiness_notes), collapse = "; "))
  }
}
...
if (length(project_notes) > 0) {
  append_triage("__PROJECT__", "info", "project_readiness_limits", paste(project_notes, collapse = "; "))
}
