...
resolve_matrix_dir <- function(cfg, sample_id, inventory_row) {
  if (is.null(inventory_row)) {
    stop(sprintf("input_inventory.tsv 缺少样本 `%s` 的记录。请先运行 workflow/03stages/audit_inputs.sh。", sample_id), call. = FALSE)
  }
  matrix_dir <- normalize_scalar_value(inventory_row$filtered_matrix_dir[1])
  if (!nzchar(matrix_dir)) {
    stop(sprintf("input_inventory.tsv 中样本 `%s` 的 filtered_matrix_dir 为空。请先运行 workflow/03stages/audit_inputs.sh 和 standardize_inputs.sh。", sample_id), call. = FALSE)
  }
