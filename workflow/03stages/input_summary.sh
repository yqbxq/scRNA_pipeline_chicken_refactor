...
if main_upstream_ready == "true" and standardized_ok:
    next_step = f"bash {pipeline_root}/workflow/03stages/install_envs.sh"
elif main_upstream_ready != "true":
    if project_input_modes == {"fastq"}:
        next_step = f"bash {pipeline_root}/workflow/03stages/alignment.sh"
    else:
        next_step = f"bash {pipeline_root}/workflow/03stages/audit_inputs.sh"
else:
    next_step = f"bash {pipeline_root}/workflow/03stages/standardize_inputs.sh"
