...
run_stage_if_stale \
  "${WORKFLOW_ROOT}/05single_script/01a_build_raw_objects.R" \
  "${MODULE_01A_MANIFEST}" \
...
run_stage_if_stale \
  "${WORKFLOW_ROOT}/05single_script/01b_pre_qc_eda.R" \
  "${MODULE_01B_MANIFEST}" \
  "${MODULE_01A_MANIFEST}" \
  "${RAW_OBJECT_OUTPUT}" \
  "${INPUT_INVENTORY_FILE}" \
...
update_workflow_status \
  ...
  "review ${EDA_GATE_FILE} and approve pre_qc before bash ${PIPELINE_ROOT}/workflow/03stages/02_qc.sh" \
