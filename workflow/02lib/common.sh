...
export ST_FANOUT_VERSION="${ST_FANOUT_VERSION:-0}"
...
export PY_CELL2LOCATION_BIN="${PY_CELL2LOCATION_BIN:-${PY_CELL2LOCATION_ENV_PREFIX}/bin/python}"
...
export CELL_COUNT_INVENTORY_THRESHOLD_OVERRIDE_TSV="${CELL_COUNT_INVENTORY_THRESHOLD_OVERRIDE_TSV:-${METADATA_DIR}/cell_count_thresholds.tsv}"
...
export H5AD_CONTRACT_FILE="${H5AD_CONTRACT_FILE:-${METADATA_DIR}/h5ad_export_contract.tsv}"
...
export MODULE_07A_CELLCHAT_VERSION="${MODULE_07A_CELLCHAT_VERSION:-${MODULE_07A_VERSION:-1.1}}"
export MODULE_07B_LIANA_VERSION="${MODULE_07B_LIANA_VERSION:-0.1}"
export MODULE_07C_NICHENET_VERSION="${MODULE_07C_NICHENET_VERSION:-1.1}"
export MODULE_07D_CONSENSUS_VERSION="${MODULE_07D_CONSENSUS_VERSION:-0.1}"
export MODULE_07E_EDA_VERSION="${MODULE_07E_EDA_VERSION:-0.1}"
...
export COMMUNICATION_REQUIRE_FULL_PIPELINE="${COMMUNICATION_REQUIRE_FULL_PIPELINE:-no}"
...
export SPATIAL_NORMALIZATION_VARIANTS_DIR="${SPATIAL_NORMALIZATION_VARIANTS_DIR:-${SPATIAL_CHECKPOINT_DIR}/normalization_variants}"
export SPATIAL_NORMALIZATION_COMPARE_DIR="${SPATIAL_NORMALIZATION_COMPARE_DIR:-${REPORT_DIR}/spatial/normalization_compare}"
export SPATIAL_NORMALIZATION_OVERRIDE_FILE="${SPATIAL_NORMALIZATION_OVERRIDE_FILE:-${PROJECT_CONFIG_DIR}/spatial_normalization_override.tsv}"
...
export SPATIAL_INTEGRATION_COMPARE_DIR="${SPATIAL_INTEGRATION_COMPARE_DIR:-${SPATIAL_INTEGRATION_REPORT_DIR}}"
...
export SPATIAL_CLUSTERING_OVERRIDE_FILE="${SPATIAL_CLUSTERING_OVERRIDE_FILE:-${PROJECT_CONFIG_DIR}/spatial_clustering_override.tsv}"
export SPATIAL_CLUSTERING_VARIANTS_DIR="${SPATIAL_CLUSTERING_VARIANTS_DIR:-${SPATIAL_CHECKPOINT_DIR}/clustering_variants}"
export SPATIAL_CLUSTERING_COMPARE_DIR="${SPATIAL_CLUSTERING_COMPARE_DIR:-${REPORT_DIR}/spatial/clustering_compare}"
export SPATIAL_CLUSTER_TARGET="${SPATIAL_CLUSTER_TARGET:-${TARGET_CLUSTERS:-8}}"
export SPATIAL_REGION_ANNOTATION_OVERRIDE_FILE="${SPATIAL_REGION_ANNOTATION_OVERRIDE_FILE:-${PROJECT_CONFIG_DIR}/spatial_region_annotation_override.tsv}"
export SPATIAL_REGION_ANNOTATION_DIR="${SPATIAL_REGION_ANNOTATION_DIR:-${SPATIAL_REGION_ANNOTATION_REPORT_DIR}}"
export SPATIAL_REGION_ANNOTATION_TABLE_DIR="${SPATIAL_REGION_ANNOTATION_TABLE_DIR:-${SPATIAL_TABLE_DIR}/spatial_region_annotation}"
export SPATIAL_SUBREGION_ANNOTATION_REPORT_DIR="${SPATIAL_SUBREGION_ANNOTATION_REPORT_DIR:-${EDA_REPORT_DIR}/spatial_subregion_annotation}"
export SPAGCN_PY_BIN="${SPAGCN_PY_BIN:-${PY_SPATIAL_LEGACY_ENV_PREFIX}/bin/python}"
export SPAGCN_PY_SCRIPT="${SPAGCN_PY_SCRIPT:-${PIPELINE_ROOT}/workflow/04python/cluster_spagcn.py}"
export STAGATE_PY_BIN="${STAGATE_PY_BIN:-${PY_SPATIAL_ENV_PREFIX}/bin/python}"
export STAGATE_PY_SCRIPT="${STAGATE_PY_SCRIPT:-${PIPELINE_ROOT}/workflow/04python/cluster_stagate.py}"
...
export SPATIAL_HVG_NFEATURES="${SPATIAL_HVG_NFEATURES:-${HVG_NFEATURES:-2000}}"
export SPATIAL_PEARSON_RESIDUALS_PY="${SPATIAL_PEARSON_RESIDUALS_PY:-${PIPELINE_ROOT}/workflow/04python/normalize_pearson_residuals.py}"
export PY_SPATIAL_BIN="${PY_SPATIAL_BIN:-${PY_SPATIAL_ENV_PREFIX}/bin/python}"
...
export SPATIAL_NEIGHBORHOOD_PY="${SPATIAL_NEIGHBORHOOD_PY:-${PIPELINE_ROOT}/workflow/04python/spatial_neighborhood.py}"
...
should_export_h5ad() {
  ...
  case "${gate_name}" in
    03d_annotation|04b_subcluster|06d_enrichment|07e_communication|\
...
ensure_eda_control_files() {
  ensure_dir \
    ...
    "${SPATIAL_INTEGRATION_COMPARE_DIR}" \
    "${SPATIAL_CLUSTERING_VARIANTS_DIR}" \
    "${SPATIAL_CLUSTERING_COMPARE_DIR}" \
    "${SPATIAL_REGION_ANNOTATION_DIR}" \
    "${SPATIAL_REGION_ANNOTATION_TABLE_DIR}" \
...
ensure_spatial_config_files() {
  ...
  if [[ ! -s "${SPATIAL_CLUSTERING_OVERRIDE_FILE}" && -f "${PIPELINE_ROOT}/config/spatial_clustering_override.tsv.template" ]]; then
    cp -f "${PIPELINE_ROOT}/config/spatial_clustering_override.tsv.template" "${SPATIAL_CLUSTERING_OVERRIDE_FILE}"
  fi
  if [[ ! -s "${SPATIAL_REGION_ANNOTATION_OVERRIDE_FILE}" && -f "${PIPELINE_ROOT}/config/spatial_region_annotation_override.tsv.template" ]]; then
  ...
  if [[ ! -s "${H5AD_CONTRACT_FILE}" && -f "${PIPELINE_ROOT}/metadata/h5ad_export_contract.tsv.template" ]]; then
    cp -f "${PIPELINE_ROOT}/metadata/h5ad_export_contract.tsv.template" "${H5AD_CONTRACT_FILE}"
  fi
  if [[ ! -s "${CELL_COUNT_INVENTORY_THRESHOLD_OVERRIDE_TSV}" && -f "${PIPELINE_ROOT}/metadata/cell_count_thresholds.tsv.template" ]]; then
    cp -f "${PIPELINE_ROOT}/metadata/cell_count_thresholds.tsv.template" "${CELL_COUNT_INVENTORY_THRESHOLD_OVERRIDE_TSV}"
  fi
}
